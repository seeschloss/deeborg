...
final class Table{
    ...
    public:
        ...
        // UPDATE
        void update( string statement, string condition, Variant[] values... ){
            string query = "UPDATE `%s` SET %s WHERE %s".format( _name, statement, condition );
            _database.command( query, values );
        }
        ...
        Variant value( string[] column, string statement, Variant[] values... ){
            string sql = "SELECT %s FROM `%s` WHERE %s".format( column.join(", "), _name, statement);
            debug writefln( "sql: %s", sql);
            Row[] results = _database.command( sql, values );
